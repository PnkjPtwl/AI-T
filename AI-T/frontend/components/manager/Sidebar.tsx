...
import { LayoutDashboard, ClipboardList, UserCircle, BookOpen, LineChart } from 'lucide-react'

const API = process.env.NEXT_PUBLIC_API_URL || 'http://localhost:4000'

const NAV_ITEMS = [
  { name: 'Dashboard', href: '/dashboard', icon: LayoutDashboard },
  { name: 'Training Management', href: '/training', icon: ClipboardList },
  { name: 'Persona Library', href: '/scenarios', icon: UserCircle },
  { name: 'Knowledge Base', href: '/knowledge-base', icon: BookOpen },
  { name: 'Team Analytics', href: '/team-analytics', icon: LineChart },
]
      ...
      <div className={`flex items-center px-5 py-5 border-b border-gray-100 h-[72px] transition-all duration-300 ${collapsed ? 'justify-center' : 'justify-start gap-3'}`}>
        <div className="flex items-center gap-0">
          ...
          <div className={`flex flex-col gap-0.5 ml-0 overflow-hidden transition-all duration-300 ${collapsed ? 'w-0 opacity-0' : 'w-[130px] opacity-100'}`}>
            ...
            <span className="hidden text-[9px] font-[700] text-purple-600 uppercase tracking-widest leading-none ml-[34px] whitespace-nowrap">{userName ? `${userName} (Manager)` : 'Manager View'}</span>
          </div>
        </div>
      </div>
